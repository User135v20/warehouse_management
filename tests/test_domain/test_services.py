...
product_list = [Product(id=1, name="test_1", quantity=1, price=1), Product(id=2, name="test_2", quantity=1, price=2),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (True, product_list[0].name, 3, 4, product_list, None,
...
def test_create_product(should_raise, name, quantity, price, product_list_test, correct_value, side_effect):
    ...
    order_repo = MagicMock()
    service = WarehouseService(product_repo, order_repo)
    with mock.patch.object(ProductRepository, 'list', return_value=product_list_test, side_effect=side_effect):
        if should_raise:
            ...
            with mock.patch.object(ProductRepository, 'add', return_value=None):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (True, [], product_list, None,
         OrderNotCreatedException(message=f"An order can't be created. Product list is empty")),
        (True, [product_list[0], product_list[0]], None, None, OrderNotCreatedException(
            message=f"An order can't be created. These products {[product_list[0]]} "
                    f"have been added more than once.")),

        (True, product_list[:1], product_list[1:], None,
         OrderNotCreatedException(products=product_list[:1])),
    ]
...
def test_get_order(should_raise, order_id, returned_value, correct_value):
    ...
    service = WarehouseService(product_repo, order_repo)
    if should_raise:
        with pytest.raises(NotFoundOrderException) as exc_info:
            with mock.patch.object(SqlAlchemyProductRepository, 'get',
                                   return_value=NoResultFound('No row was found when one was required')):
...
def test_create_product(should_raise, name, quantity, price, product_list_test, correct_value, side_effect):
    ...
    if should_raise:
        ...
        with mock.patch.object(ProductRepository, 'list', return_value=product_list_test):
            with mock.patch.object(ProductRepository, 'add', return_value=None):
                assert service.create_product(name, quantity, price) == correct_value
